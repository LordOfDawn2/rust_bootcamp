...
fn generate_map(size_str: &str) -> Result<Vec<Vec<u8>>, String> {
...
fn animate_pathfinding(
    ...
    println!("\nStep {}: Exploring ({},{}) - cost: {}", step, current_pos.1, current_pos.0, cost);
...
fn dijkstra_min(
    grid: &[Vec<u8>],
    animate: bool,
) -> (Vec<(usize, usize)>, u32, Vec<(usize, usize)>) {
    let height = grid.len();
    let width = grid[0].len();
    ...
    while let Some(State { cost, pos }) = heap.pop() {
        ...
        if animate && (visited_order.len() == 1 || visited_order.len() == 2 || visited_order.len() % 10 == 0) {
